...
#'
#' @return 
#' An object of class \code{xgb.cv2.synchronous} with the following elements:
#' \itemize{
...
#' cv <- xgb.cv2(data = dtrain, nrounds = 3, nthread = 2, nfold = 5, metrics = list("rmse","auc"),
#'                   max_depth = 3, eta = 1, objective = "binary:logistic")
#' print(cv)
#' print(cv, verbose=TRUE)
#' 
#' @export
xgb.cv2 <- function(params=list(), data_pairs, nrounds, label = NULL, missing = NA,
...
#' Print xgb.cv2 result
#' 
#' Prints formatted results of \code{xgb.cv2}.
#' 
#' @param x an \code{xgb.cv2.synchronous} object
...
#' cv <- xgb.cv2(data = train$data, label = train$label, nfold = 5, max_depth = 2,
#'                eta = 1, nthread = 2, nrounds = 2, objective = "binary:logistic")
#' print(cv)
#' print(cv, verbose=TRUE)
#' 
#' @rdname print.xgb.cv2
#' @method print xgb.cv2.synchronous
#' @export
print.xgb.cv2.synchronous <- function(x, verbose = FALSE, ...) {
  cat('##### xgb.cv2 ', length(x$folds), '-folds\n', sep = '')
  ...
  if (verbose) {
    ...
    if (!is.null(x$params)) {
      cat('params (as set within xgb.cv2):\n')
      cat( '  ', 
